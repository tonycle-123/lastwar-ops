...
function NameCell({ name, priorDates }: { name: string; priorDates: string[] }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <span style={{ fontWeight: 700, color: '#1a2a4a' }}>{name}</span>
      {hasFlag && (
        <button onClick={() => setOpen(o => !o)} title="Active in last 14 days"
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: 13, lineHeight: 1 }}>
          ⚠️
        </button>
      )}
      {hasFlag && open && (
        <span style={{ fontSize: 11, color: '#c07040', whiteSpace: 'nowrap' }}>{priorDates.map(shortDate).join(', ')}</span>
      )}
    </div>
  )
}
...
function MemberAutocomplete({ label, required, selectedId, selectedName, members, onSelect, onManualChange, recentDates }: {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <input
        className="lw-input"
        style={{ borderColor: hasWarning ? '#f0a080' : undefined }}
      ...
      />
      {hasWarning && <p style={{ color: '#c07040', fontSize: 11, marginTop: 4 }}>⚠️ Active in last 14 days: {recentDates!.map(shortDate).join(', ')}</p>}
      {open && filtered.length > 0 && (
        <div className="lw-dropdown">
          {filtered.map(m => (
            <button key={m.id} type="button" className="lw-dropdown-item"
              onMouseDown={() => { onSelect(m.id, m.name); setQuery(m.name); setOpen(false) }}>
              <span className={`rank-badge rank-${m.rank}`} style={{ fontSize: 9, padding: '1px 5px' }}>R{m.rank}</span>
              <span>{m.name}</span>
            </button>
          ))}
        </div>
...
export default function TrainPage() {
  ...
  function getRecentDatesFor(name: string): string[] {
    return recentLogs
      ...
      .map(l => l.log_date).filter((v, i, a) => a.indexOf(v) === i)
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <div className="section-title" style={{ marginBottom: 4 }}>
            <i className="ti ti-train" aria-hidden="true" /> Train Conductor Log
          </div>
          <div style={{ fontSize: 12, color: '#8090b8' }}>{logs.length} days recorded</div>
        </div>
        <button className="btn-primary btn-sm" onClick={openAdd}>
          <i className="ti ti-plus" aria-hidden="true" style={{ fontSize: 12 }} /> Log Today
        </button>
      </div>
      ...
      {showForm && (
        <div className="lw-form-panel" style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#1a2a4a', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{editId ? 'Edit Entry' : 'Log Train Entry'}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
            ...
            <MemberAutocomplete label="Conductor" required selectedId={form.conductor_id} selectedName={form.conductor_name} members={members} recentDates={conductorRecentDates}
            ...
            <MemberAutocomplete label="VIP / Special Guest" selectedId={form.vip_id} selectedName={form.vip_name} members={members} recentDates={vipRecentDates}
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            <button className="btn-primary btn-sm" onClick={handleSave} disabled={saving}>{saving ? 'Saving…' : editId ? 'Save Changes' : 'Save Entry'}</button>
            <button className="btn-primary btn-sm" onClick={() => { setShowForm(false); setError(null) }} style={{ background: 'transparent', color: '#4a5a7a', borderColor: '#c0cce0' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        <div style={{ textAlign: 'center', color: '#8090b8', padding: '48px 0', fontSize: 13 }}>Loading log…</div>
      ) : filtered.length === 0 ? (
                ...
                return (
                  <tr key={log.id}>
                    <td style={{ whiteSpace: 'nowrap' }}>
                      <span style={{ color: '#4a5a7a', fontSize: 12 }}>{formatDate(log.log_date)}</span>
                      {isRecent && <span style={{ marginLeft: 6, background: '#e8f0ff', border: '1px solid #5a8ae8', color: '#1a4ab0', fontSize: 9, padding: '1px 6px', borderRadius: 4, fontWeight: 700 }}>RECENT</span>}
                    </td>
                    ...
                    <td style={{ color: '#8090b8', fontSize: 12 }}>{log.notes || '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 5, justifyContent: 'flex-end' }}>
                        <button className="btn-primary btn-sm" onClick={() => openEdit(log)}>Edit</button>
                        <button className="btn-danger btn-sm" onClick={() => handleDelete(log.id, log.log_date)}>Delete</button>
                      </div>
                    </td>
                  </tr>
                )
